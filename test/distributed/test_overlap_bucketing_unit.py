...
class TestOverlapSchedulingNoMemoryLimit(InductorTestCase):
    ...
    @parametrize(
        ...
        [
            ...
            # controls that must keep scheduling exactly as they did before.
            subtest({}, name="defaults"),
            subtest({"max_memory_increase_gb": None}, name="gb_none"),
            subtest({"max_memory_increase_ratio": None}, name="ratio_none"),
            subtest(
                {"max_memory_increase_gb": None, "max_memory_increase_ratio": None},
                name="both_none",
            ),
        ],
    )
    def test_memory_caps_schedule(self, caps):
        node_names = self._schedule(**caps)
        self.assertIn("all_gather_into_tensor", node_names)
        self.assertIn("wait_tensor", node_names)
    ...
    def test_no_memory_limit_tracker_is_inert(self):
        from torch._inductor.fx_passes.memory_estimator import NoOpMemoryTracker
